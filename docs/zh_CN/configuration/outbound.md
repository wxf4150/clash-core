...
Clash 通过向指定的 URL 发送 HTTP HEAD 请求来测量列表中每个代理服务器的**网络延迟（响应时间）**。延迟测试会在启动时自动执行一次，然后根据 `interval` 设置（单位为秒）定期执行。可以设置最大容忍值、测试间隔和目标 URL。

该策略组会自动选择延迟最低的代理服务器。当设置 `lazy: true` 时，健康检查仅在策略组被实际使用时运行（在间隔周期内）。
